...
//Make sure all registers match for loads and stores
int checkmemrefargs(struct assemline *line, int rulenumber, int linenum);
...
void readinrules()
{
   ...
   //If we picked up an extra newline at end of file, creating an empty rule
   if(!Rules[numpeeprules].nummatchlines)
      numpeeprules--;
...
struct assemline* replacelines(struct assemline *line, int rulenumber)
{
   ...
   if(!noprev)
   {
      replaceline = insline(prevline->blk,nextline,newline);
   }
   else
   {
   ...
   if(replaceline->next && (replaceline->next->type == COMMENT_LINE ||
      replaceline->next->type == DEFINE_LINE))
   {
      if(!(replaceline->prev && (replaceline->prev->type == COMMENT_LINE ||
         replaceline->prev->type == DEFINE_LINE)))
      {
         nextline = replaceline->next;
         unhookline(nextline);
         hookupline(replaceline->blk,replaceline,nextline);
...
int deadsmatch(struct assemline *line, int rulenumber, int linenum)
{
   ...
   while(ruletoken)
   {
      insreg(Rules[rulenumber].arguments[atoi(ruletoken)],v,
               calcregpos(Rules[rulenumber].arguments[atoi(ruletoken)]));
      ruletoken = strtok(NULL,"%");
   }
...
//Make sure arguments match for loads and stores
int checkmemrefargs(struct assemline *line, int rulenumber, int linenum)
{
   struct Line *curmatchline;
   curmatchline = Rules[rulenumber].matchlines[linenum];
   ...
   if(line->type == LOAD_INST)
   {

   }

   if(line->type == STORE_INST)
   {
      //Bracketed argument second
...
//Make sure arguments match between matchline and assemline
int regsmatch(struct assemline *line, int rulenumber, int linenum)
{
   ...
   if(curmatchline->memref)
   {
      if(checkmemrefargs(line,rulenumber,linenum))
         return 1;
      else
         return 0;
   }
...
struct Line* parseline(char* line)
{
   ...
   while(token != NULL)
   {
      ...
      if(tokencount == 0)
      {
         ...
         //If we have a memory access instruction, treat it differently
         if(strstr(token,"st") || strstr(token,"ld"))
            curline->memref = 1;
   ...
   //Parse arguments for non-memory matchline
   if(!curline->memref)
   {
   ...
   //Parse arguments for memory instruction
   else
   {
      //Parse arguments for load
      if(strstr(curline->instruction,"ld"))
      {
         token = strtok(curline->arguments, ",");
         curline->argument1 = allocstring(token);
         token = strtok(NULL, ",");
      ...
      //Parse arguments for store
      if(strstr(curline->instruction,"st"))
      {
         token = strtok(curline->arguments, ",");
         curline->argument1 = allocstring(token+1);
         token = strtok(NULL, ",");
