...
! source statement 123
	...
	sethi	%hi(btab),%g3       ! sets=%g3:    uses=         deads=
!
! block 2
! preds: 1 20
! succs: 16
!  doms: 1 2
!   ins=%g2:%g3:%g4:%g7:%o3:%o4:%o5:
...
! source statement 125
	...
	mov	%g3,%g3             ! sets=%g3:    uses=%g3:     deads=%g3:
...
!
.L94:
	smul	%g3,7,%o0           ! sets=%o0:    uses=%g3:     deads=
	add	%g3,%lo(btab),%o1   ! sets=%o1:    uses=%g3:     deads=%g3:
...
! source statement 176
	add	%g2,%lo(.L111),%o0  ! sets=%o0:    uses=%g2:     deads=
	mov	256,%o1             ! sets=%o1:    uses=         deads=
	add	%g2,%lo(_iob),%o2   ! sets=%o2:    uses=%g2:     deads=%g2:
...
! source statement 177
	mov	%g1,%l0             ! sets=%l0:    uses=%g1:     deads=
	add	%g1,%lo(.L111),%o0  ! sets=%o0:    uses=%g1:     deads=%g1:
	mov	%l1,%o1             ! sets=%o1:    uses=%l1:     deads=
	mov	%l3,%o2             ! sets=%o2:    uses=%l3:     deads=
	mov	%l2,%o3             ! sets=%o3:    uses=%l2:     deads=
	call	mkline,4            ! sets=scratch uses=%o0:%o1:%o2:%o3:deads=%o0:%o1:%o2:%o3:
