...
%type <addr_ptr> src, mem, dst, dst0, ddememaddr, memaddr, addr,  expr, simpexpr, hreg
%type <addr_ptr> regexp, iregexp, label, con, immed, reg, ireg
%type <str_ptr> rel frel
%%

inst  :  mips_inst { if(dde) erflag=1; printf("PRINT\n");}
   | dde_inst { if(!dde) erflag=1; printf("PRINT\n");}
   |  '@' '=' ';'
   ;

dde_inst :  dde_bb				{ bb($1); }
	|	dde_bcf				{ bc($1, 'f'); }
	|	dde_bct				{ bc($1, 't'); }
	|	dde_binst 				{ binst($1); }
	|	dde_cmpset				{ cmpset($1); }
	...
	|	dde_li				{ li($1); }
	|	dde_mfc				{ mfc($1); }
	|	dde_mfu dde_mfl				{ mfu($1, $2); }
	|	dde_mfl dde_mfu				{ mfu($2, $1); }
	|	dde_mov				{ mov($1); }
	|	dde_mtc				{ mtc($1); }
   		|       dde_mfhi                    { mfhi($1); }
      	|       dde_mfireg                    { mfireg($1); }
   ...
   dde_bb	:	PC '=' dst0 rel dst0 ',' label ';'
   					{ $$ = rrecord($4, $3, $5, $7); };
   dde_bcf	:	PC '=' FC ':' '0' ',' label ';'	{ $$ = record($7, AN, AN); };
   dde_bct	:	PC '=' FC ':' '1' ',' label ';'	{ $$ = record($7, AN, AN); };
   dde_nop	:	FC '=' FC ';'
   	|	';';
   dde_binst	:	dst '=' rhs ';'			{ $$ = binopi($1, $3); };
   dde_cmpset	:	dst '=' dst rel src ';'		{ $$ = rrecord($4, $1, $3, $5); };
   dde_cvt	:	dst '=' CV '[' dst ']' ';'	{ $$ = record($1, $5, AN);};
   dde_cvtfi	:	dst '=' TT '[' dst ']' ';'	{ $$ = record($1, $5, AN);};
   dde_cvtif	:	dst '=' WY '[' dst ']' ';'	{ $$ = record($1, $5, AN);};

   dde_divide :        LO '=' reg '/' reg ';'          { $$ = record($3, $5, AN); }
           |       HI '=' reg '/' reg ';'                  { $$ = record($3, $5, AN); };
   dde_divideu :       LO '=' reg '\\' reg ';'         { $$ = record($3, $5, AN); }
           |       HI '=' reg '\\' reg ';'                 { $$ = record($3, $5, AN); };

   dde_high    :       dst '=' HI'['GLOBAL']'';'        { $$ = record($1, addr(ANY, GIDENT, AE $5, AN, AN), AN); }
   	|       dst '=' HI'['immed']'';'         { $$ = record($1, $5, AN); };

   dde_low     :       dst '=' dst'|'LO'['GLOBAL']'';'  { $$ = record($1, $3, addr(ANY, GIDENT, AE $7, AN, AN)); }
           |       dst '=' dst'|'LO'['immed']'';'   { $$ = record($1, $3, $7); };

   dde_hilo	: 	dst '=' HI '[' GLOBAL ']' '|' LO '[' GLOBAL ']' ';' { $$ = record($1, addr(ANY, GIDENT, AE $5, AN, AN), AN); }
   	| 	dst '=' HI '[' immed ']' '|' LO '[' immed ']' ';' { $$ = record($1, $5, AN); }
           ;


   dde_brk     :       BK ';'                          { $$ = record(AN,AN,AN); };

   dde_multi   :       LO '=' reg '*' reg ';'          { $$ = record($3, $5, AN);  };

   /* Only accept if the arguments match.  Instead of creating a new type with 6 parameters,
      the comparison is done here. */
   dde_multu   :       ireg '=' reg '*' reg '"' con ';' ireg '=' reg '*' reg ';'      { $$ = record(icompare($3, $11) ? $3 : 0, icompare($5,$13) ? $5 : 0, $7);  };


   dde_fcmp	:	FC '=' dst frel src ';'		{ $$ = rrecord($4, $3, $5, AN); };
   dde_jal	:	ST '=' addr ';'  		{ $$ = record($3, AN, AN); }
   ...
   dde_jmp	:	PC '=' expr ';'			{ $$ = record($3, AN, AN); }
   	|	PC '=' reg ';'			{ $$ = record($3, AN, AN); };
   dde_l	:	dst '=' mem ';'			{ $$ = record($1, $3, AN); }
   	|	dst '=' mem '&' con ';'		{ $$ = record($1, $3, $5); }
   	|	dst '=' '(' mem '{' con ')' '}' con ';'
   				{ $$ = record($1, $4, same($6, $9)); };
   dde_li	:	dst '=' simpexpr ';'		{ $$ = record($1, $3, AN); };
   dde_mfc	:	dst '=' MY '[' dst  ']' ';'	{ $$ = record($1, $5, AN); };
   dde_mfu	:	dst '=' MU '[' dst  ']' ';'	{ $$ = record($1, $5, AN); };
   dde_mfl	:	dst '=' ML '[' dst  ']' ';'	{ $$ = record($1, $5, AN); };
   dde_mov	:	dst '=' dst ';'			{ $$ = record($1, $3, AN); };
   dde_mtc	:	dst '=' MT '[' dst  ']' ';'	{ $$ = record($1, $5, AN); };



   dde_hreg	: 	REG3 { $$ = addr(LONG, HREGISTER, AE $<str_ptr>1, AN, AN); };
   dde_cvtil	: 	dst '=' IL '[' dst ',' dst ']' ';' { $$ = record($1, $5, $7); };

   dde_mfhi    :       dst '=' HI ';'                  { $$ = record($1, AN, AN); };
   dde_mflo    :       dst '=' LO ';'                  { $$ = record($1, AN, AN); };
   dde_mfireg    :       dst '=' ireg ';'                  { $$ = record($1, $3, AN); };

   dde_neg	:	dst '=' '-' dst ';'		{ $$ = record($1, $4, AN); };
   dde_not	:	dst '=' '~' dst ';'		{ $$ = record($1, $4, AN); };
   dde_ret	:	PC '=' RT ';'
   dde_s	:	mem '=' dst0 ';'		{ $$ = record($1, $3, AN); };
   dde_samemem	:	mem '=' mem ';'			{ $$ = record($1, $3, AN); };
   dde_mtof    :       dst '=' WY '[' MT '[' dst ']' ']' ';'      { $$ = record($1, $7, AN); };
   dde_mtoi    :       dst '=' MY '[' TT '[' dst ']' ']' ';'      { $$ = record($1, $7, AN); };
   dde_rhs	:	dst '+' src		{ $$ = brecord(ADDI, $1, $3); }
...
/*
  dde_dst0	:	dst
   ...
   dst	:   REG0	{ $$ =addr(LONG, RREGISTER, AE $<str_ptr>1, AN, AN); }
      ...
      dde_mem	: BMEM memaddr ']'	{ $$ = memref(BYTE, $2); }
...
*/
   ...
   dde_dst:  MMEM ddememaddr ']' { $$ = memref(WORD, $2); }
   	;

   dde_immed	:	expr		{ $$ = addr(ANY, IMMED, $1, AN, AN); }
   	;

   dde_memaddr: ireg { $$=addr(ANY, REGDFR, $1, AN, AN);}
            | iregexp { $$ = displacement($1); }
            ;

   dde_addr	:	expr		{ $$ = addr(ANY, ABSOLUTE, $1, AN, AN); }
           |	reg		{ $$ = addr(ANY, REGDFR, $1, AN, AN); }
           |	regexp		{ $$ = displacement($1); }
           ;

   dde_iregexp	:	ireg '+' expr		{ $$ = initexpr($1, 0, $3); }
           	|	ireg '-' expr		{ $$ = initexpr($1, 1, $3); }
           	|	expr '+' ireg		{ $$ = initexpr($3, 0, $1); }
           	|	iregexp '+' expr		{ $$ = addexpr($1, ADDI, $3); }
           	|	iregexp '-' expr		{ $$ = addexpr($1, SUBI, $3); }
           	|	'(' iregexp ')'		{ $$ = $2; }
           	;

   dde_regexp	:	reg '+' expr		{ $$ = initexpr($1, 0, $3); }
   ...
   dde_reg	: REG0	{ $$ = addr(LONG, RREGISTER, AE $<str_ptr>1, AN, AN); }
      ;

   dde_ireg	:	REG4	{ $$ = addr(LONG, RREGISTER, AE $<str_ptr>1, AN, AN); };


   dde_simpexpr:       expr '+' expr   { $$ = addr(ANY, EXPR, AE ADDI, $1, $3); }
   ...
   dde_expr	:	 expr '+' expr	{ $$ = addr(ANY, EXPR, AE ADDI, $1, $3); }
   	|	expr '-' expr	{ $$ = addr(ANY, EXPR, AE SUBI, $1, $3); }
   ...
   dde_label	:	LBL	{ $$ = addr(LONG, LABEL, AE $1, AE yylength, AN); }
   	;

   dde_con	:	CON	{ $$ = addr(ANY, ICONST, AE $1, AE yylength, AN); }
...
ddedst:  MMEM ddememaddr ']' { $$ = memref(WORD, $2); }
	;
...
memaddr	:	/*expr		{ $$ = addr(ANY, ABSOLUTE, $1, AN, AN); }
        |*/	reg		{ $$ = addr(ANY, REGDFR, $1, AN, AN); }
        |	regexp		{ $$ = displacement($1); }
        |   ireg  { $$ = addr(LONG, RREGISTER, AE $<str_ptr>1, AN, AN); };
        ;

ddememaddr: ireg { $$=addr(ANY, REGDFR, $1, AN, AN);}
         | iregexp { $$ = displacement($1); }
         ;
...
iregexp	:	ireg '+' expr		{ $$ = initexpr($1, 0, $3); }
        	|	ireg '-' expr		{ $$ = initexpr($1, 1, $3); }
        	|	expr '+' ireg		{ $$ = initexpr($3, 0, $1); }
        	|	iregexp '+' expr		{ $$ = addexpr($1, ADDI, $3); }
        	|	iregexp '-' expr		{ $$ = addexpr($1, SUBI, $3); }
