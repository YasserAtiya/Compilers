...
! source statement 5
	...
	ldd	[%o0 + %lo(.L01)],%f0! sets=%f0:%f1:uses=%o0:     deads=
	std	%f0,[%fp + .1_x]    ! sets=x:      uses=%fp:%f0:%f1:deads=
! source statement 6
	sethi	%hi(.L15),%o0       ! sets=%o0:    uses=         deads=
	ldd	[%fp + .1_x],%f0    ! sets=%f0:%f1:uses=%fp:x:   deads=
	ldd	[%fp + .1_x],%f2    ! sets=%f2:%f3:uses=%fp:x:   deads=
	add	%o0,%lo(.L15),%o0   ! sets=%o0:    uses=%o0:     deads=
	ldd	[%fp + .1_x],%f4    ! sets=%f4:%f5:uses=%fp:x:   deads=
	std	%f4,[%sp + 72]      ! sets=        uses=%sp:%f4:%f5:deads=
	ld	[%sp + 72],%o1      ! sets=%o1:    uses=%sp:     deads=
	ld	[%sp + 76],%o2      ! sets=%o2:    uses=%sp:     deads=
	fmuld	%f0,%f2,%f0         ! sets=%f0:%f1:uses=%f0:%f1:%f2:%f3:deads=
	std	%f0,[%sp + 80]      ! sets=        uses=%sp:%f0:%f1:deads=
	ld	[%sp + 80],%o3      ! sets=%o3:    uses=%sp:     deads=
	ld	[%sp + 84],%o4      ! sets=%o4:    uses=%sp:     deads=
	call	printf,5            ! sets=scratch uses=%o0:%o1:%o2:%o3:%o4:deads=
	nop
!
! block 2
! preds: 1
