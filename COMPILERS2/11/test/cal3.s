...
! source statement 34
	...
	mov	%l3,%o1             ! sets=%o1:    uses=%l3:     deads=
...
number:
	...
	!.2_str = %g4
	mov	%i0,%g4             ! sets=%g4:    uses=%i0:     deads=%i0:
! source statement 78
...
cal:
	...
	!.4_y = %l4
	mov	%i1,%l4             ! sets=%l4:    uses=%i1:     deads=%i1:
	!.4_p = %l5
	mov	%i2,%l5             ! sets=%l5:    uses=%i2:     deads=%i2:
	!.4_w = %l6
...
!
	...
	mov	%o2,%l2             ! sets=%l2:    uses=%o2:     deads=%o2:
	stb	%o0,[%o1]           ! sets=        uses=%o0:%o1: deads=%o0:%o1:
...
jan1:
	...
	save	%sp,(-104),%sp      ! sets=window  uses=%sp:     deads=
	!.5_yr = %l3
	mov	%i0,%l3             ! sets=%l3:    uses=%i0:     deads=
! source statement 184
	mov	%l3,%o0             ! sets=%o0:    uses=%l3:     deads=%l3:
	mov	%o0,%l1             ! sets=%l1:    uses=%o0:     deads=%o0:
! source statement 185
	mov	%l1,%l0             ! sets=%l0:    uses=%l1:     deads=
