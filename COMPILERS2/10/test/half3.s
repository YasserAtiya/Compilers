...
.1_i = 68
	st	%i0,[%fp + .1_i]    ! sets=i:      uses=%i0:%fp: deads=%i0:
! source statement 6
	...
	std	%f0,[%fp + .1_d]    ! sets=d:      uses=%fp:%f0:%f1:deads=%f0:%f1:
! source statement 7
	ldd	[%fp + .1_d],%f0    ! sets=%f0:%f1:uses=%fp:d:   deads=d:
...
!
	...
	ld	[%fp + .2_n],%o1    ! sets=%o1:    uses=%fp:n:   deads=n:
