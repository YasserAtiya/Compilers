...
struct assemline* replacelines(struct assemline *line, int rulenumber)
{
   ...
   if(!Rules[rulenumber].replacelines[0]->wildline)
   {
      ...
      if(Rules[rulenumber].replacelines[0]->argument1)
      {
         ...
         {
            //fprintf(stderr, "Replacing with lit in: %s\n",curline->text);
...
int regsmatch(struct assemline *line, int rulenumber, int linenum)
{
   ...
   {
      //fprintf(stderr, "Match Line: %s has literal in it\n",curmatchline->text);
      //fprintf(stderr, "Line: %s\n",line->text);
      //fprintf(stderr, "MatchLine arg: %s\n",curmatchline->argument1);
      //fprintf(stderr, "Line arg: %s\n",line->items[1]);
   ...
   if(curmatchline->arg2lit)
   {
      //fprintf(stderr, "Match Line: %s has literal in it\n",curmatchline->text);
      //fprintf(stderr, "Line: %s\n",line->text);
      //fprintf(stderr, "MatchLine arg: %s\n",curmatchline->argument2);
      //fprintf(stderr, "Line arg: %s\n",line->items[2]);
      if(strcmp(curmatchline->argument2,line->items[2]) != 0)
         return 0;

         //fprintf(stderr, "They match2!\n");

   }
   else
   {
      //fprintf(stderr, "Arg2 not lit\n");
   ...
   while(token != NULL)
   {
      ...
      if(tokencount == 0)
      {
         ...
         if(!strstr(token,"$"))
         {
            ...
            //fprintf(stderr, "Argument 1 of line is lit: %s\n",curline->text);
         }
      }
      else if(tokencount == 1)
      {
